...
module.exports = function(RED){ 
    ...
    function irFallPrevent(config) {
        ...
        if (checkPin.getDigitalPinValue(key)==1){
            ...
            console.log('IrFallPrevent digital pin ' + node.digitalPin +' OK');
        }
        else{
    	...
    	{
            var value = irf.read();
            node.status({fill: "red", shape: "dot", text: "IR Fall Prevent value is " + value});
            console.log("Ir Fall prevent value is " + value);
    		var msg = { payload:value };
    		node.send(msg);
    	}
    }
    RED.nodes.registerType("DF-IrFallPrevent", irFallPrevent);
}
