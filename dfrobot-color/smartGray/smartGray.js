...
module.exports = function(RED){ 
    ...
    function smartGraySensor(config) {
        ...
        if (checkPin.getDigitalPinValue(key)==1){
            ...
            console.log('SmartGraySensor digital pin ' + node.digitalPin +' OK');
        }
        else{
    	...
    	{
            var value = irf.read();
            node.status({fill: "red", shape: "dot", text: "Smart Gray Scale sensor value is " + value});
            console.log("Smart Gray Scale sensor value is " + value);
    		var msg = { payload:value };
    		node.send(msg);
    	}
    }
    RED.nodes.registerType("DF-SmartGraySensor", smartGraySensor);
}
